...
train_ER_list = zeros(1, num_epochs);
valid_ER_list = zeros(1, num_epochs);
test_ER_list = zeros(1, num_epochs);
...
for epoch = 1:num_epochs
  ...
  train_ER = mean((prediction < 0.5 & target_train == 1) | (prediction > 0.5 & target_train == 0));
  % Compute deriv
  ...
  valid_ER = mean((prediction < 0.5 & target_valid == 1) | (prediction > 0.5 & target_valid == 0));
  ...
  train_ER_list(1, epoch) = train_ER;
  valid_ER_list(1, epoch) = valid_ER;
  test_CE_list(1, epoch) = test_CE;
  test_ER_list(1, epoch) = test_ER;
%   fprintf(1,'%d  Train CE=%f, Valid CE=%f\n',...
%             total_epochs, train_CE, valid_CE);
%   fprintf(1,'%d  Train CE=%f, Test CE=%f\n',...
%             total_epochs, train_CE, test_CE);
  fprintf(1,'%d  Train ER=%f, Valid ER=%f\n',...
            total_epochs, train_ER, valid_ER);
  fprintf(1,'%d  Train ER=%f, Test ER=%f\n',...
            total_epochs, train_ER, test_ER);
end
...
% train_errors(1, start_epoch : total_epochs) = train_ER_list;
% valid_errors(1, start_epoch : total_epochs) = valid_ER_list;
%   hold on, ...
%   plot(epochs(1, 1 : total_epochs), train_errors(1, 1 : total_epochs), 'b'),...
%   plot(epochs(1, 1 : total_epochs), valid_errors(1, 1 : total_epochs), 'g'),...
%   legend('Train', 'Test'),...
